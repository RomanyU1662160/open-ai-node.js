...
const abacus_client = new OpenAI({
...
const getAbacusChatResponse = async (
  content: string,
  ...
  if (stream) {
    const chat_completion = await abacus_client.chat.completions.create({
      model: abacus_model,
      messages: messages,
      stream: true, //
    ...
    const completion = await abacus_client.chat.completions.create({
      model: abacus_model,
      messages: messages,
      stream: false,
    });
